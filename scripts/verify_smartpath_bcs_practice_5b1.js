...
assert.equal(new Set(mirror.items.map(item => item.id)).size, 180);
const allDestinations = router.mistakeDestinations(mirror.items, graph).concat(destinations.destinations);
assert.equal(allDestinations.filter(destination => destination.item_id).length, 180);
...
console.log(JSON.stringify({status:'PASS',newReviewedItems:70,existingItems:30,preservedPhase5B1Items:100,currentTotalMappedItems:180,canonicalSkillsCovered:new Set(pack.map(item => item.micro_skill)).size,goals:goals.goal_ids.length}, null, 2));
