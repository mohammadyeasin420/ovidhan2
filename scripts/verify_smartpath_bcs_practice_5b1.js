...
assert.equal(new Set(graph.item_mappings.map(mapping => mapping.item_id)).size, 100);
...
assert.equal(mirror.items.length, 100);
assert.equal(new Set(mirror.items.map(item => item.id)).size, 100);
const allDestinations = router.mistakeDestinations(mirror.items, graph).concat(destinations.destinations);
assert.equal(allDestinations.filter(destination => destination.item_id).length, 100);
...
console.log(JSON.stringify({status:'PASS',newReviewedItems:70,existingItems:30,totalMappedItems:100,canonicalSkillsCovered:new Set(pack.map(item => item.micro_skill)).size,goals:goals.goal_ids.length}, null, 2));
