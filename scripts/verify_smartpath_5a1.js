...
assert.equal(graph.graph_version,3); assert.equal(graph.families.length,14); assert.equal(graph.skills.length,65); assert.equal(graph.item_mappings.length,240);
assert.ok(graph.families.some(x=>x.family_id==='WRITING'));
assert.deepEqual(new Set(['writing_precis','formal_letter_writing']),new Set(graph.skills.filter(x=>['writing_precis','formal_letter_writing'].includes(x.id)).map(x=>x.id)));
...
console.log(JSON.stringify({status:'PASS',graph:{families:14,skills:65,itemMappings:240},transferPatterns:4,goals:6,writingDestinations:2},null,2));
