...
const { items, addReviewedPack, addLiteraturePack, addWrittenPack } = require('../mistake-mirror.js');
...
addWrittenPack(require('../data/bcs-written-smartpath-v1.json'));
...
test('graph preserves original nodes and extends to 14 families and 65 skills',()=>{assert.equal(graph.graph_version,3);assert.equal(graph.families.length,14);assert.equal(graph.skills.length,65);assert.ok(graph.families.some(x=>x.family_id==='WRITING'));assert.ok(graph.families.some(x=>x.family_id==='LITERATURE'));assert.ok(graph.families.some(x=>x.family_id==='READING_COMPREHENSION'));assert.ok(graph.families.some(x=>x.family_id==='TRANSLATION'));});
test('all 240 reviewed items map once to canonical skill and family',()=>{assert.equal(graph.item_mappings.length,240);assert.deepEqual(new Set(graph.item_mappings.map(x=>x.item_id)),new Set(items.map(x=>x.id)));});
