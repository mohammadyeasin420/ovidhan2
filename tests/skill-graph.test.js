...
test('graph preserves original nodes and extends to 11 families and 52 skills',()=>{assert.equal(graph.graph_version,2);assert.equal(graph.families.length,11);assert.equal(graph.skills.length,52);assert.ok(graph.families.some(x=>x.family_id==='WRITING'));assert.ok(graph.skills.some(x=>x.id==='writing_precis'));assert.ok(graph.skills.some(x=>x.id==='formal_letter_writing'));});
test('all 100 reviewed items map once to canonical skill and family',()=>{assert.equal(graph.item_mappings.length,100);assert.deepEqual(new Set(graph.item_mappings.map(x=>x.item_id)),new Set(items.map(x=>x.id)));});
